...
func (p *Poller) UrgentTrigger(fd, connIndex, taskType int, arg interface{}) (err error) {
	task := queue.GetTask()
	task.Fd, task.ConnIndex, task.TaskType, task.Arg = fd, connIndex, taskType, arg
...
func (p *Poller) Polling(taskFind func(task *queue.Task) error, callback func(fd int, filter int16) error) error {
