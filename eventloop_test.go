...
func TestServeGC(t *testing.T) {
	...
	t.Run("gc-loop", func(t *testing.T) {
		...
		t.Run("4-loop-200000", func(t *testing.T) {
			testServeGC(t, "tcp", ":9000", true, true, 4, 200000)
		})
...
func (s *testServerGC) OnBoot(eng Engine) (action Action) {
	...
	go func() {
		...
		eng.eng.lb.iterate(func(idx int, e *eventloop) bool {
			...
			log.Println("el:", idx, e.connCount)
			return true
		})

		var d, dc time.Duration
		start := time.Now()
